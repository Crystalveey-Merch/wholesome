...
export const EditRuleModal = ({
  ...
  const handleUpdateRule = async () => {
    ...
    try {
      ...
      const updatedRules = formerRules.map((r) =>
        r.sortNumber === updatedRule.sortNumber ? updatedRule : r
      );

      await updateDoc(doc(db, "interests", interest.id), {
        rules: updatedRules,
      });
      setInterest({ ...interest, rules: updatedRules });
      toast.success("Rule updated successfully");
      setOpen(false);
    } catch (error) {
      toast.error("An error occurred while updating rule: " + error.message);
    } finally {
      setUpdating(false); // Ensure to set the updating state to false in both success and error cases
    }
  };
            ...
            >
              ...
              <button
                ...
                className={`h-11 w-max justify-self-end place-self-end px-3 py-2 bg-red-500 text-white font-inter font-semibold text-sm rounded-md focus:outline-none focus:ring-0 transition duration-300 ease-in-out ${
                  updating || !isTitleAccepted || !isDescriptionAccepted
